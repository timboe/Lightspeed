...
public class DopplerObject {
	...
	void CalculateColour() {
		...
		double my_axis_velocity = Math.sqrt((my_vx_axis*my_vx_axis) + (my_vy_axis*my_vy_axis));
		
		double pl_vx_axis = U.player.vx * Math.sin(angle);
		double pl_vy_axis = U.player.vy * Math.cos(angle);
		double pl_axis_velocity = Math.sqrt((pl_vx_axis*pl_vx_axis) + (pl_vy_axis*pl_vy_axis));
